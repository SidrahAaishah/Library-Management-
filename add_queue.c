...
struct shelf
{
    ...
    int row;
} shelf_arr[max_shelf]; // shelf array
int num_shelf = 0;
void add_book_to_shelf(struct book *b)
{
    int i = 0, flag = 0;
    for (i = 0; i < num_shelf; i++)
    {
        if (shelf_arr[i].col < shelf_size - 1 && shelf_arr[i].row < shelf_size - 1)
        {
            flag = 1;
            break;
        }
    ...
    {
        b->shelf_n = i + 1;
        if (shelf_arr[i].col < shelf_size - 1)
        {
            shelf_arr[i].col++;
            b->r = shelf_arr[i].row;
            b->c = shelf_arr[i].col;
            shelf_arr[i].a[shelf_arr[i].row][shelf_arr[i].col] = b;
        }
        else
        {
            shelf_arr[i].row++;
    ...
    {
        if (num_shelf == max_shelf)
        {
            printf("Library has reached it's limit to store books\n\n");
            return;
        }
        else
        {
            ...
            b->r = 0;
            b->c = 0;
            shelf_arr[num_shelf - 1].row = 0;
            shelf_arr[num_shelf - 1].col = 0;
            shelf_arr[num_shelf - 1].a[0][0] = b;
        }
    }
}
void add_book()
